...
def get_oa_link(paper, doi=None, only_unpaywall=True):
    ...
    if doi:
        ...
        # T354472: Reduce chances of incorrect title matches on PMC
            if (landing_page.startswith("https://europepmc.org") or landing_page.startswith("https://www.ncbi.nlm.nih.gov/pmc/")) and oa_location.get("evidence") is "oa repository (via OAI-PMH title and first author match)" and int(resp.get("year", 2000)) < 2000:
                continue
